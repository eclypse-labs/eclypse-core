...
contract PositionsManager is Ownable, IPositionsManager {
	...
	function collRatioOf(uint256 _tokenId) public view returns (uint256) {
		...
		uint256 fees = FullMath.mulDiv(fee0, protocolContracts.priceFeed.getPrice(positionFromTokenId[_tokenId].token0, Denominations.ETH), 1e18) +
			FullMath.mulDiv(fee1, protocolContracts.priceFeed.getPrice(positionFromTokenId[_tokenId].token1, Denominations.ETH), 1e18);

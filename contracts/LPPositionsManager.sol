...
contract LPPositionsManager is ILPPositionsManager, Ownable, Test {
	...
	function debtOf(uint256 _tokenId) public view override returns (uint256 currentDebt) {
		uint debtPrincipal = getPosition(_tokenId).debt;
		currentDebt = debtPrincipal * interestConstant / getPosition(_tokenId).interestConstant;
		uint newInterest = lessDumbPower(interestRate, block.timestamp - interestConstantTimestamp);
		currentDebt = FullMath.mulDivRoundingUp(currentDebt, newInterest, FixedPoint96.Q96);
	}
	...
	modifier onlyBOorAP() {
		require(
			msg.sender == borrowerOperationsAddress || msg.sender == address(activePool),
	...
	modifier onlyBOorLPPM() {
		require(
			msg.sender == address(this) || msg.sender == borrowerOperationsAddress,

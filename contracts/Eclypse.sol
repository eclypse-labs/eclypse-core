...
contract Eclypse is IEclypse, Ownable {
	...
	function changePositionStatus(uint256 _tokenId, Status _status) public onlyBorrowerOperations {
		require(positionFromTokenId[_tokenId].status != _status, "A position status cannot be changed to its current one.");
	...
	function openPosition(address _owner, uint256 _tokenId) public override onlyBorrowerOperations {
		(, , address token0, address token1, uint24 fee, int24 tickLower, int24 tickUpper, uint128 liquidity, , , , ) = uniswapPositionsNFT.positions(
			_tokenId
		);
	...
	function positionAmounts(uint256 _tokenId) public view override returns (uint256 amountToken0, uint256 amountToken1) {
		...
		(uint256 amount0, uint256 amount1) = LiquidityAmounts.getAmountsForLiquidity(sqrtRatioX96, sqrtRatio0X96, sqrtRatio1X96, _position.liquidity);
	 ...
	 */
	function increaseDebtOf(uint256 _tokenId, uint256 _amount) public override onlyBorrowerOperations onlyActivePosition(_tokenId) {
		require(_amount > 0, "A debt cannot be increased by a negative amount or by 0.");

		uint256 debtPrincipal = getPosition(_tokenId).debtPrincipal;
		uint256 currentDebt = FullMath.mulDivRoundingUp(debtPrincipal, protocolValues.interestFactor, getPosition(_tokenId).interestConstant);
		uint256 newInterestFactor = lessDumbPower(protocolValues.interestRate, block.timestamp - protocolValues.lastFactorUpdate);
		currentDebt = FullMath.mulDivRoundingUp(currentDebt, newInterestFactor, FixedPoint96.Q96);
		protocolValues.totalBorrowedGho += _amount;
		protocolValues.interestFactor = FullMath.mulDivRoundingUp(protocolValues.interestFactor, newInterestFactor, FixedPoint96.Q96);
	 ...
	 */
	function decreaseDebtOf(uint256 _tokenId, uint256 _amount) public override onlyBorrowerOperations onlyActivePosition(_tokenId) {
		...
		emit DecreasedDebt(positionFromTokenId[_tokenId].user, _tokenId, oldDebt, debtPrincipal + accumulatedInterest - _amount);
	}
	...
	function priceInETH(address _tokenAddress) public view override returns (uint256 priceX96) {
		...
		(int24 twappedTick, ) = OracleLibrary.consult(tokenToWETHPoolInfo[_tokenAddress].poolAddress, protocolValues.twapLength);
		uint160 sqrtRatioX96 = TickMath.getSqrtRatioAtTick(twappedTick);
		priceX96 = FullMath.mulDiv(sqrtRatioX96, sqrtRatioX96, FixedPoint96.Q96);
		if (tokenToWETHPoolInfo[_tokenAddress].inv) priceX96 = FullMath.mulDiv(FixedPoint96.Q96, FixedPoint96.Q96, priceX96);
	}
	...
	function positionValueInETH(uint256 _tokenId) public view override returns (uint256 value) {
		...
		return FullMath.mulDiv(amount0, priceInETH(token0), FixedPoint96.Q96) + FullMath.mulDiv(amount1, priceInETH(token1), FixedPoint96.Q96);
	}
	 ...
	 */
	function setNewLiquidity(uint256 _tokenId, uint128 _liquidity) public onlyBorrowerOperations onlyActivePosition(_tokenId) {
	 ...
	 */
	function mintPosition(INonfungiblePositionManager.MintParams memory params) public override onlyBorrowerOperations returns (uint256 tokenId) {
	...
	function decreaseLiquidity(
		...
		_liquidityToRemove = _liquidityToRemove > getPosition(_tokenId).liquidity ? getPosition(_tokenId).liquidity : _liquidityToRemove;
	 ...
	 * @dev Only the Borrower Operations contract or the LP Positions Manager contract can call this function.
	 */
	function repayDebtFromUserToProtocol(
		address sender,
		uint256 amount,
		uint256 tokenId
	) external override onlyBorrowerOperations {

...
contract BorrowerOperations is Ownable, CheckContract, IBorrowerOperations, ReentrancyGuard {
	...
	function openPosition(uint256 _tokenId) external positionNotInitiated(_tokenId) {
		...
		ContractsCache memory contractsCache = ContractsCache(eclypse, GHO);
		contractsCache.eclypse.openPosition(msg.sender, _tokenId);
	 ...
	 */
	function closePosition(uint256 _tokenId) public onlyActivePosition(_tokenId) onlyPositionOwner(_tokenId, msg.sender) {
		uint256 debt = eclypse.debtOf(_tokenId);

		if (!(debt == 0)) {
			revert Errors.DebtIsNotPaid(debt);
		}

		eclypse.sendPosition(msg.sender, _tokenId);

		eclypse.changePositionStatus(_tokenId, IEclypse.Status.closedByOwner);

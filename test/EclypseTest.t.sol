...
contract EclypseTest is UniswapTest {
	...
	function testRiskConstantsAreCorrectlyUpdated() public {
		...
		assertEq(eclypse.getRiskConstants(address(uniPoolUsdcETHAddr)), _minCR, "Risk constants are not updated correctly.");
	}
	...
	function testliquidatePosition() public {
		...
		assertEq(uint256(eclypse.getPosition(facticeUser1_tokenId).status), 3, "Position should be closed by liquidation.");

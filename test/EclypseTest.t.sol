...
contract EclypseTest is UniswapTest {
    ...
    //test DebtOf - passes when debt= 0
    ...
    // test DebtOf/increaseDebtOf - doesnt pass
    //the problem for now is that debtOf gives a wrong value, due to the wrong update of interestConstant
    function testDebtWhenDebt1() public {
        ...
        assertEq(
            ...
            300 * TOKEN18,
            "Position should have 300 GHO debt. (debt principal)"
        );
        uint256 currentDebt = eclypse.debtOf(facticeUser1_tokenId);
    ...
    //does not pass
    //same problem but here in decreaseDebtOf, the interestConstant is wrongly updated
    ...
    // passes
    //user can close his position when he has not borrowed
    function testPositionStatus_closeByOwner() public {
        assertEq(uint256(eclypse.getPosition(facticeUser1_tokenId).status), 1, "Position should be active.");

        assertEq(uint256(eclypse.getPosition(facticeUser1_tokenId).debtPrincipal), 0, "user has not borrowed");
    ...
    function testPositionStatus_notExistent() public {
        assertEq(uint256(eclypse.getPosition(0).status), 0, "Position should not exist.");
    }
    ...
    function testRiskConstantsAreCorrectlyUpdated() public {
        ...
        assertEq(
            eclypse.getRiskConstants(address(uniPoolUsdcETHAddr)), _minCR, "Risk constants are not updated correctly."
        );
    }

    function testRiskConstant_increase() public {
        uint256 minRC = FullMath.mulDiv(13, FixedPoint96.Q96, 10);
        eclypse.updateRiskConstants(address(uniPoolUsdcETHAddr), minRC);

        uint256 initialMinRC = eclypse.getRiskConstants(address(uniPoolUsdcETHAddr));

        assertEq(eclypse.getRiskConstants(address(uniPoolUsdcETHAddr)), minRC, "Risk constant should be updated.");

        uint256 newMinRC = FullMath.mulDiv(15, FixedPoint96.Q96, 10);
        eclypse.updateRiskConstants(address(uniPoolUsdcETHAddr), newMinRC);

        uint256 EndMinRC = eclypse.getRiskConstants(address(uniPoolUsdcETHAddr));

        assertEq(eclypse.getRiskConstants(address(uniPoolUsdcETHAddr)), newMinRC, "Risk constant should be updated.");

        assertGt(EndMinRC, initialMinRC, "Risk constant should be updated and increased.");
    }
    ...
    function testRiskConstant_decrease() public {
        ...
        eclypse.updateRiskConstants(address(uniPoolUsdcETHAddr), setInitialRC);
        uint256 initialRC = eclypse.getRiskConstants(address(uniPoolUsdcETHAddr));

        eclypse.updateRiskConstants(address(uniPoolUsdcETHAddr), setNewMinRC);
        uint256 newMinRC = eclypse.getRiskConstants(address(uniPoolUsdcETHAddr));
    ...
    function testLiquidatable_positionNotLiquidatable() public {
        ...
        uint256 cr = eclypse.collRatioOf(facticeUser1_tokenId);
    ...
    function testliquidatePosition() public {
        ...
        assertEq(
            uint256(eclypse.getPosition(facticeUser1_tokenId).status), 3, "Position should be closed by liquidation."
        );

...
contract CompleteTest is Test {
    ...
    address public constant uniPoolUsdcETHAddr = 0xE0554a476A092703abdB3Ef35c80e0D76d32939F;
    ...
    function setUp() public {
        ...
        (user1_tokenId1,,,) =
            uniswapPositionsNFT.mint(createMintParams(100, 115135, 276324, 280989067, 249999999487800429, user1));
        (user1_tokenId2,,,) =
            uniswapPositionsNFT.mint(createMintParams(100, 184216, 230270, 339911608, 149407050671375578, user1));
        (user1_tokenId3,,,) = uniswapPositionsNFT.mint(createMintParams(100, 205335, 207243, 46637557137, 0, user1));
        ...
        (user2_tokenId1,,,) =
            uniswapPositionsNFT.mint(createMintParams(100, 204805, 207243, 263090524275, 64156357155675129377, user2));
        (user2_tokenId2,,,) =
            uniswapPositionsNFT.mint(createMintParams(100, 204642, 207243, 346797719887, 424424889204713414, user2));
        (user2_tokenId3,,,) =
            uniswapPositionsNFT.mint(createMintParams(100, 204619, 207243, 8147401333, 3142772096131753480, user2));
        ...
        (user3_tokenId1,,,) =
            uniswapPositionsNFT.mint(createMintParams(100, 204361, 207243, 402251943327, 21723060789068296769, user3));
        (user3_tokenId2,,,) =
            uniswapPositionsNFT.mint(createMintParams(100, 203878, 207243, 14339519322, 10799999999468255542, user3));
        (user3_tokenId3,,,) =
            uniswapPositionsNFT.mint(createMintParams(100, 203465, 207243, 411353548280, 38175199687068858458, user3));
        ...
        (user4_tokenId1,,,) =
            uniswapPositionsNFT.mint(createMintParams(100, 202763, 207243, 466435657866, 3744921609026570929, user4));
        (user4_tokenId2,,,) =
            uniswapPositionsNFT.mint(createMintParams(100, 202667, 207243, 50000 * TOKEN6, 798277255604665973, user4));
        (user4_tokenId3,,,) =
            uniswapPositionsNFT.mint(createMintParams(100, 201936, 207243, 12052947528, 8324207920310153428, user4));
        ...
        (user5_tokenId1,,,) =
            uniswapPositionsNFT.mint(createMintParams(100, 201935, 207243, 3706008306, 5705995494298136638, user5));
        (user5_tokenId2,,,) =
            uniswapPositionsNFT.mint(createMintParams(100, 200311, 207243, 50000 * TOKEN6, 10587577356159341, user5));

        (user5_tokenId3,,,) =
            uniswapPositionsNFT.mint(createMintParams(100, 200311, 207243, 8215000547, 17795864747568551189, user5));
    ...
    function createMintParams(uint24 fees, int24 lower, int24 upper, uint256 amount0, uint256 amount1, address sender)
        public
        view
    ...
    function createEthGhoPool() private {
        ...
        PoolGHO_WETH = IUniswapV3Pool(uniswapFactory.createPool(address(ghoToken), address(WETH), 500));
        ...
        if (PoolGHO_WETH.token0() == address(ghoToken)) {
            PoolGHO_WETH.initialize(uint160(FullMath.mulDiv(FixedPoint96.Q96, 1, 35))); // 1 ETH = 1225 GHO
            ...
            PoolGHO_WETH.initialize(uint160(FullMath.mulDiv(FixedPoint96.Q96, 35, 1))); // 1 ETH = 1225 GHO
            mintParams = INonfungiblePositionManager.MintParams({
                token0: address(WETH),
        ...
        lpPositionsManager.addPairToProtocol(
            address(PoolGHO_WETH), address(ghoToken), wethAddr, address(PoolGHO_WETH), address(0), false, true
        );
    ...
    function testComplete() public {
        ...
        assertEq(
            activePool.getMintedSupply(), GHOAmount + GHOAmount, "GHOAmount1_1 + GHOAmount1_2 should have been minted."
        );
        assertEq(
        ...
        assertEq(
            activePool.getMintedSupply(), GHOAmount3 + GHOAmount, "GHOAmount1 + GHOAmount2_1 should have been minted."
        );
        assertEq(
        ...
        assertEq(
            activePool.getMintedSupply(),
            GHOAmount3 + GHOAmount3 + GHOAmount3 + GHOAmount3 + GHOAmount + GHOAmount,
        ...
        assertGt(
            lpPositionsManager.totalDebtOf(user1), initialDebtUser1, "The total debt of user1 should have increased."
        );
        ...
        assertEq(
            lpPositionsManager.totalDebtOf(user2), 3183624 * 10 ** 13, "The total debt of user2 should be 31.83624 GHO."
        );
